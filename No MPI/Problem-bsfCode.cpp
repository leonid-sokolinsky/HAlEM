...
Theme: HAlEM - Hybrid Along Edges Movement method (No MPI)
...
void PC_bsf_Init(bool* success) {
	...
	* success = MTX_LoadVector(PD_v, PP_MTX_POSTFIX_V);
	...
	PD_iterNo = 0;
	Vector_MultiplyByNumber(PD_c, PP_LAUNCH_VECTOR_LENGTH / Vector_Norm(PD_c), PD_launchVector);

	List_equations(PD_isEquation, PD_eqHyperplanes, &PD_meq);
	MakeEquationsBasis(PD_basis_v, &PD_meq_basis, PP_EPS_INVERSE);
	...
	int rank;
	Matrix_Rank(PD_neHyperplanes_v, PD_mne_v, &rank, PP_EPS_INVERSE);
	...
	if (BSF_sv_mpiRank == BSF_sv_mpiMaster) {
		...
		if (!*success)
			return;
		Matrix_Rank(PD_basis_v, PD_n, &rank, PP_EPS_INVERSE);
		if (rank < PD_n) {
			if (BSF_sv_mpiRank == BSF_sv_mpiMaster)
				cout << "PC_bsf_Init error: Loaded bases has rank less then n with precision of PP_EPS_INVERSE = "
				<< PP_EPS_INVERSE << endl;
			*success = false;
			return;
		}
	...
	Vector_Copy(PD_v, PD_v_nex);
	PD_objF_nex = ObjF(PD_v_nex);
	Vector_i_Copy(PD_basis_v, PD_basis_v_nex);
}
...
void PC_bsf_MapF(PT_bsf_mapElem_T* mapElem, PT_bsf_reduceElem_T* reduceElem, int* mapSuccess) {
	double* v_cur = BSF_sv_parameter.v;	// Current vertex
	double objF_u_cur = ObjF(v_cur);
	int* basis_v = BSF_sv_parameter.basis_v;
	PT_vector_T  d;	// Unit direction vector
	PT_vector_i_T edgeBasis;
	PT_bitscale_T basisBitscale;
	...
	cout << "--------------------------------------------------------------------------------------------------------------------" << endl;
	// Vector_Print(v_cur, PP_EPS_ZERO);
	//cout << "\n"; Vector_i_Print(basis_v); cout << endl;
	#endif // _DEBUG /**/
	...
	#else // !PP_GRADIENT
	if (objF_nex > reduceElem->objF_nex + PP_EPS_ZERO) {
	...
	//cout << "\n u_nex_max = "; Vector_Print(reduceElem->v_nex, PP_EPS_ZERO); cout << endl;
	#endif // _DEBUG /**/
} // end PC_bsf_MapF
...
void PC_bsf_ParametersOutput(PT_bsf_parameter_T parameter) {
	...
	cout << "PP_EPS_INVERSE\t\t" << PP_EPS_INVERSE << endl;
	cout << "PP_EPS_ON_HYPERPLANE\t" << PP_EPS_ON_HYPERPLANE << endl;
	cout << "PP_LAUNCH_VECTOR_LENGTH\t" << PP_LAUNCH_VECTOR_LENGTH << endl;
	#ifdef PP_MIN_COS
	cout << "PP_MIN_COS\t\t" << PP_MIN_COS << endl;
	#endif // PP_MIN_COS
...
void PC_bsf_ProcessResults(PT_bsf_reduceElem_T* reduceResult, int reduceCounter, PT_bsf_parameter_T* parameter, int* nextJob, bool* exit) {
	...
	if (Exit(PD_v_nex, reduceResult->v_nex, &exit_code)) {
		...
		case 2:
			...
			break;
		default:
			cout << "Unpredictable exit code!" << endl;
		}
...
namespace PF {
	...
	static inline void CheckBelongnessToPolytope(double* v, bool* exit) {
		if (!PointBelongsToPolytope(v, PP_EPS_ON_HYPERPLANE)) {
			cout << "PC_bsf_ProcessResults error: vertex does not belong to polytope with precision of PP_EPS_ON_HYPERPLANE = "
				<< PP_EPS_ON_HYPERPLANE << endl;
			double eps_on_polytope = PP_EPS_ON_HYPERPLANE;
			Tuning_Eps_PointBelongsToPolytope(v, &eps_on_polytope);
			cout << "Minimum PP_EPS_ON_HYPERPLANE should be " << eps_on_polytope << endl;
			*exit = true;
			return;
		}
	...
	static inline void CheckRank(int* neHyperplanes_v, int mne_v, bool* exit, double eps_inverse) {
		int rank;
		Matrix_Rank(neHyperplanes_v, mne_v, &rank, eps_inverse);
		if (rank < PD_subspaceDim)
			*exit = true;
	}

	static inline bool Exit(double* v, double* v_nex, int* exit_code) {
		double objF_nex = ObjF(v_nex);

		#ifdef PP_MAX_OBJ_VALUE
		if (RelativeError(PP_MAX_OBJ_VALUE, objF_nex) < PP_EPS_RELATIVE_ERROR) {
			*exit_code = 1;
			return true;
		}
		#endif // PP_MAX_OBJ_VALUE
	...
	static inline void GetUnitDirectionVector(double* v, int* edgeBasis, double* launchVector, double* d, bool* success) {
		...
		if (!*success) {
			//#ifdef _DEBUG
			cout << "Worker " << BSF_sv_mpiRank
				<< ": PC_bsf_MapF:GetUnitDirectionVector: Not possible to create inverse matrix for direct orthogonal projection. "
				<< "Try to define PP_EPS_INVERSE." << endl;
			//#endif // _DEBUG /**/
			return;
		}
	...
	static inline void InsertIntoBasis(int item, int* basis, int i, bool* success, double eps_inverse) {
		...
		int rank;
		Matrix_Rank(basis, _n, &rank, eps_inverse);
		if (rank == _n)
			*success = true;
		else
			*success = false;
	...
	static inline void Jump(PT_vector_T startPoint, PT_vector_T jumpVector, PT_vector_T finishPoint, bool* parallelHPlanes, int* success, double eps_zero) {
		...
		for (int i = 0; i < _m; i++) {
			...
			norm_a_DOT_norm_d = a_DOT_d / _norm_a[i];

			if (norm_a_DOT_norm_d < eps_zero)
				continue;

			b_MINUS_a_DOT_z = _b[i] - Vector_DotProduct(_A[i], z);

			lambda = b_MINUS_a_DOT_z / a_DOT_d;
		...
		Vector_MultiplyByNumber(d, lambda_min, lambda_min_DOT_d);
		Vector_Addition(startPoint, lambda_min_DOT_d, finishPoint);
		*success = true;
	}
	...
	static inline void MakeBasis_v(int* neHyperplanes_v, int mne_v, int meq_basis, int* basis_v, double eps_inverse) {
		int m_v = meq_basis;
	...
	static inline void MakeEdgeBasis(int* basis_v, int i_toExclude, int* edgeBasis) {
		for (int j = 0; j < i_toExclude; j++)
			edgeBasis[j] = basis_v[j];

		for (int j = i_toExclude; j < PD_n - 1; j++)
			edgeBasis[j] = basis_v[j + 1];
	}

	static inline void MakeEquationsBasis(int* eqBasis, int* m_eqBasis, double eps_inverse) {
		...
		*m_eqBasis = 0;
		...
		for (int i = 1; i < PD_meq; i++) {
			AddToBasis(PD_eqHyperplanes[i], eqBasis, *m_eqBasis, &success, eps_inverse);
			if (success)
				(*m_eqBasis)++;
		}
	}

	static inline void MakeRndBasis_v(int* neHyperplanes_v, int mne_v, int meq_basis, int* basis_v, double eps_inverse) {
		int m_v = meq_basis;
		PT_bitscale_T bitscale;

		Bitscale_False(bitscale, mne_v);
	...
	static inline void Ort_DDTI(int m, bool* success, double eps_inverse) {
		double factor;
		...
		*success = true;
		...
		for (int j = 0; j < m; j++) {
			...
			if (fabs(PD_DDT[j][j]) <= eps_inverse) { // Search for a non-zero below
				int i_ne_0 = -1;
				for (int i = j + 1; i < m; i++) {
					if (i_ne_0 == -1) {
						if (fabs(PD_DDT[i][j]) > eps_inverse)
							i_ne_0 = i;
					}
					else {
				...
				if (i_ne_0 > -1) { // Permute the rows
					...
					for (int lj = j + 1; lj < m; lj++) {
						if (j_ne_0 == -1) {
							if (fabs(PD_DDT[j][lj]) > eps_inverse)
								j_ne_0 = lj;
						}
						else {
	...
	static inline void OrtProjecting(int* flatHyperplanes, int m_flat, PT_vector_T z, PT_vector_T w, bool* success,
		...
		Ort_DDTI(m_flat, success, eps_inverse);
	...
	static inline bool PointIsVertex(PT_vector_T v, double eps_inverse, double eps_on_hyperplane) {
		...
		Matrix_Rank(hyperplanes_v, mh_v, &rank, eps_inverse);
		if (rank < PD_n)
			return false;
		else
			return true;
	...
	static inline void BASIS_Make_A0(int* basis_v) {
		for (int i = 0; i < _n; i++)
			for (int j = 0; j < _n; j++)
				PD_A0[i][j] = _A[basis_v[i]][j];
	}

	static inline void BASIS_Make_A0I(bool* success, double eps_inverse) {
		double factor;
	...
	static inline void BASIS_Make_cA0I(PT_vector_T cA0I) {
		for (int j = 0; j < _n; j++) {
			cA0I[j] = 0;
			for (int i = 0; i < _n; i++)
	...
	static inline void BASIS_Make_u(PT_column_T u, int* basis_v, PT_vector_T cA0I, double eps_zero) {
		Column_Zeroing(u);
	...
	static inline void BASIS_Make_y(int* basis_v, int i_star, PT_vector_T y) {
		bool ok = false;
	...
	static inline void Basis_Scroll(PT_vector_T v_nex, PT_vector_i_T basis_v_nex, double* lambda, bool* success) {
		...
		BASIS_Make_A0I(success, PP_EPS_INVERSE);
		BASIS_Make_cA0I(cA0I);
		...
		if (*lambda < PP_EPS_ZERO) {
			for (int j = 0; j < _n; j++) { // Replacing basis
				if (basis_v_nex[j] == i_star) {
					basis_v_nex[j] = j_star;
					break;
				}
			}
		}
